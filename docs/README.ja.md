...
> [!WARNING]
>
> pgisletはPostgreSQL 17をサポートします。任意のPostgreSQL管理コマンドを許可する汎用SQLプロキシではありません。ロールと権限によるオブジェクトの分離を提供しますが、ユーザーごとに別のデータベースや専用のCPU、メモリ、ディスクリソースを割り当てるものではありません。コンテナやVMレベルの分離は提供しません。
...
リポジトリには、サンプル用PostgreSQLインスタンスを起動するためのCompose設定が含まれています。
...
一時的なPostgreSQL 17コンテナを起動し、次の動作を示します。
...
テストはTestcontainersでPostgreSQL 17を作成・終了します。指定された`PGISLET_DSN`で既存データベースを対象にする構造ではありません。デフォルトモードではDockerの準備に失敗するとテストも失敗します。
...
### PostgreSQL 18は使用できますか？

現在の実装は17.xのみを許可します。他のバージョンへの対応には、単にバージョン検証を削除するのではなく、そのバージョンでパーサー、ポリシー、カタログ、権限の動作を検証する必要があります。
